...
def test_pathologist_review_generator(tmp_path: Path):
    ...
    html_path = generator.generate_html_review_sheet(
        case_id="test_case",
        patient_id="pt_123",
        predicted_class="Invasive Carcinoma",
        confidence=0.92,
    ...
    assert (tmp_path / "scoring_test_case.json").exists()

    content = html_path.read_text(encoding="utf-8")
    assert "GCIA Audit - Validation Clinique Histologique" in content
    assert "Invasive Carcinoma" in content
