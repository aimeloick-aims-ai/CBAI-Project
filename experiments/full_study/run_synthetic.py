...
def main():
    ...
    out = ROOT / "reports/full_study/synthetic_v1"
    out.mkdir(parents=True, exist_ok=False)
    sources = [config_path, Path(__file__), Path(__file__).with_name("synthetic.py")]
        ...
        for seed in config["seeds"]:
            ...
            for c, concept in enumerate(config["concepts"]):
                ...
                for layer, (htrain, htest) in enumerate(zip(hs_train, hs_test, strict=True)):
                    probe = make_pipeline(
                        StandardScaler(), LogisticRegression(C=1, max_iter=1000, random_state=seed)
                    )
                    probe.fit(htrain.numpy(), ytrain)
                ...
                scaler = final_probe.named_steps["standardscaler"]
                coefficient = final_probe.named_steps["logisticregression"].coef_[0] / scaler.scale_
                norm = np.linalg.norm(coefficient)
                direction = torch.tensor(coefficient / max(norm, 1e-12), dtype=torch.float32)
                # Linear head makes the directional logit derivative exact.
                tcav = (
                    float(((model.head.weight[predicted] @ direction) > 1e-8).float().mean())
                    if norm > 1e-10
                    else None
                )
    ...
    lines += [f"| {s['architecture']} | {s['seed']} | {s['accuracy']:.3f} |" for s in scores]
    lines += [
        "",
